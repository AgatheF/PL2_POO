...
public class HelloImageServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        // type de reponse image et non text/html
        response.setContentType("image/png;charset=UTF-8");
        // try ( instruction){ ...} instruction entre parenthèse fermée automatiquement si erreur
        //ici = sortie (défois = connexion par exemple --> pas besoin de finally)
        // changer le type de sortie out ici image pas objet printWriter
        try (OutputStream out = response.getOutputStream()) {

            int nbRepet = Integer.parseInt(request.getParameter("nombre"));
            String texte = request.getParameter("texte");
            int size=400;
            Color backgroundColor=Color.white;
            Color textColor=Color.BLUE;
            
           BufferedImage monImage=createImage(size, texte, nbRepet,backgroundColor,textColor);
           ImageIO.write(monImage,"png", out); // ecrit l'image sur la sortie
        }
    }
